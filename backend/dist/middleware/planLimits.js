...
exports.checkUserPlanAccess = checkUserPlanAccess;
...
async function getCurrentUsage(userId, action) {
    ...
    switch (action) {
        case "message":
            return await index_1.prisma.message.count({
                where: {
                    senderId: userId,
                    createdAt: {
                        gte: today,
        ...
        case "contactRequest":
            return await index_1.prisma.match.count({
                where: {
...
const checkPlanLimits = (options) => {
    return async (req, res, next) => {
        try {
            ...
            const userLimits = limits[planRole];
            if (!userLimits) {
            ...
            if (limit !== -1 && currentUsage >= limit) {
                console.log(`User ${userId} exceeded ${action} limit: ${currentUsage}/${limit} (plan: ${planType}, role: ${planRole})`);
            ...
            console.log(`Plan check passed for user ${userId}: ${action} (${currentUsage}/${limit === -1 ? "∞" : limit})`);
            next();
        }
        catch (error) {
...
async function checkUserPlanAccess(userId, action) {
    try {
        const planStatus = await getUserPlanStatus(userId);
        const { planType, planRole, isExpired } = planStatus;
        const limits = PLAN_LIMITS[planType];
        ...
        return {
            ...
            reason: limit !== -1 && currentUsage >= limit
