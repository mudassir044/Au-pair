...
router.post('/', async (req, res) => {
    try {
        ...
        const { targetUserId, startDate, endDate, totalHours, hourlyRate, currency, notes } = req.body;
        // Validation
        ...
        const conflictingBooking = await index_1.prisma.booking.findFirst({
            where: {
...
router.delete('/:bookingId', async (req, res) => {
    try {
        ...
        await index_1.prisma.booking.delete({
            where: { id: bookingId }
        });
...
router.get('/au-pair/:auPairId/availability', async (req, res) => {
    try {
        ...
        const auPair = await index_1.prisma.user.findUnique({
        ...
        const bookedSlots = await index_1.prisma.booking.findMany({
            where: whereClause,
            select: {
                id: true,
                startDate: true,
