...
router.get("/search", auth_1.authenticate, async (req, res) => {
    try {
        ...
        // Search parameters
        const { country, minBudget, maxBudget, languages, availability, skills, experienceLevel, } = req.query;
        // Determine opposite role to search for
        const targetRole = userRole === "AU_PAIR" ? "HOST_FAMILY" : "AU_PAIR";
        const profileTable = targetRole === "AU_PAIR" ? "au_pair_profiles" : "host_family_profiles";
      ...
      `)
        ...
        if (minBudget && targetRole === "HOST_FAMILY") {
            query = query.gte("profile.maxBudget", parseFloat(minBudget));
        }
        if (maxBudget && targetRole === "AU_PAIR") {
            query = query.lte("profile.hourlyRate", parseFloat(maxBudget));
        }
        if (languages) {
        ...
        const existingMatchIds = new Set(existingMatches?.map((match) => userRole === "AU_PAIR" ? match.hostId : match.auPairId) || []);
...
router.get("/:userId", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const profileTable = user.role === "AU_PAIR" ? "au_pair_profiles" : "host_family_profiles";
        ...
        const hostId = requestingUserRole === "HOST_FAMILY" ? requestingUserId : userId;
        const auPairId = requestingUserRole === "AU_PAIR" ? requestingUserId : userId;
...
router.get("/recommendations/for-me", auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const userRole = req.user.role;
        const limit = parseInt(req.query.limit) || 10;
        // Get current user's profile to base recommendations on
        const currentUserProfileTable = userRole === "AU_PAIR" ? "au_pair_profiles" : "host_family_profiles";
        const { data: currentProfile, error: currentProfileError } = await supabase_1.supabase
        ...
        const targetRole = userRole === "AU_PAIR" ? "HOST_FAMILY" : "AU_PAIR";
        const targetProfileTable = targetRole === "AU_PAIR" ? "au_pair_profiles" : "host_family_profiles";
        // Get potential matches
        const { data: potentialMatches, error } = await supabase_1.supabase
            .from("users")
            .select(`
        id, email, role, createdAt,
        ...
        const scoredMatches = potentialMatches
            ...
            if (userRole === "AU_PAIR" &&
                ...
                const preferredCountries = currentProfile.preferredCountries
                    .toLowerCase()
                    .split(",");
                if (preferredCountries.some((country) => profile.country.toLowerCase().includes(country.trim()))) {
                    score += 30;
                }
            }
            ...
            if (currentProfile.languages && profile.preferredLanguages) {
                ...
                const preferredLanguages = profile.preferredLanguages
                    .toLowerCase()
                    .split(",");
                const commonLanguages = userLanguages.filter((lang) => preferredLanguages.some((prefLang) => prefLang.trim().includes(lang.trim())));
