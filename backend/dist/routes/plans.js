...
router.post("/upgrade", auth_1.authenticate, async (req, res) => {
    try {
        ...
        // Validate add-ons
        if (addOns.length > 0) {
            const invalidAddOns = addOns.filter((addon) => !ALLOWED_ADDONS.includes(addon));
            ...
            if (!PLAN_CONFIG[planType].addOnsAllowed) {
                return res.status(400).json({
        ...
        if (config.duration && typeof config.duration === "object") {
            const durationDays = config.duration[role];
            planExpiry = new Date();
        ...
        // Log the upgrade
        console.log(`Plan upgrade: User ${userId} upgraded to ${planType} (${role}) with addOns: [${addOns.join(", ")}], expires: ${planExpiry}`);
        res.json({
            ...
            planDetails: {
                ...
                expiry: planExpiry,
                addOns: addOns,
                durationDays: config.duration && typeof config.duration === "object"
                    ? config.duration[role]
                    : null,
            },
        });
    }
