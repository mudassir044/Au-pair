...
router.get("/stats", auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (!user.profilecompleted) {
            let profileData, profileError, requiredFields;
            ...
            if (!profileError && profileData) {
                ...
                stats.profileCompletion = Math.round((completedFields / requiredFields.length) * 100);
            }
        }
        // Get match statistics based on role
        const matchField = userRole === "AU_PAIR" ? "auPairId" : "hostId";
        // Pending matches
        const { count: pendingCount, error: pendingError } = await supabase_1.supabase
            .from("matches")
        ...
        // Approved matches
        const { count: approvedCount, error: approvedError } = await supabase_1.supabase
            .from("matches")
            .select("*", { count: "exact", head: true })
            .eq(matchField, userId)
            .eq("status", "APPROVED");
        ...
        const { count: bookingsCount, error: bookingsError } = await supabase_1.supabase
            .from("bookings")
            .select("*", { count: "exact", head: true })
            .eq(matchField, userId)
            .eq("status", "APPROVED")
        ...
        // Total messages
        const { count: totalMsgCount, error: totalMsgError } = await supabase_1.supabase
            .from("messages")
            .select("*", { count: "exact", head: true })
        ...
        // Unread messages
        const { count: unreadCount, error: unreadError } = await supabase_1.supabase
            .from("messages")
            .select("*", { count: "exact", head: true })
