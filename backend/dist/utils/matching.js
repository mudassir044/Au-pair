...
exports.findMatches = exports.calculateMatchScore = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
const calculateMatchScore = (auPairProfile, hostProfile) => {
    let score = 0;
    let maxScore = 100;
    // Language matching (30 points)
    const languageScore = calculateLanguageMatch(auPairProfile.languages || [], hostProfile.preferredLanguages || []);
    ...
    const availabilityScore = calculateAvailabilityMatch(auPairProfile.availableFrom, auPairProfile.availableTo, new Date() // Current date as host's preferred start
    );
    score += availabilityScore * 0.15;
    // Budget matching (10 points)
    const budgetScore = calculateBudgetMatch(auPairProfile.hourlyRate, hostProfile.maxBudget);
    score += budgetScore * 0.1;
    return Math.round(score);
};
exports.calculateMatchScore = calculateMatchScore;
const calculateLanguageMatch = (auPairLangs, hostPreferredLangs) => {
    if (hostPreferredLangs.length === 0)
        return 100; // No preference means all are acceptable
    const commonLanguages = auPairLangs.filter(lang => hostPreferredLangs.some(preferred => preferred.toLowerCase() === lang.toLowerCase()));
    return (commonLanguages.length / hostPreferredLangs.length) * 100;
};
const calculateAgeMatch = (auPairBirthDate, childrenAges) => {
    if (!auPairBirthDate || childrenAges.length === 0)
    ...
    if (hasYoungChildren && auPairAge >= 18 && auPairAge <= 30)
        return 100;
    if (hasTeens && auPairAge >= 20 && auPairAge <= 35)
        return 100;
    if (auPairAge >= 18 && auPairAge <= 35)
        return 70; // Generally acceptable
    return 30; // Outside preferred age range
};
const calculateAvailabilityMatch = (auPairFrom, auPairTo, hostPreferredStart) => {
    if (!auPairFrom || !auPairTo || !hostPreferredStart)
        return 50; // Neutral score
    ...
    if (daysDiff <= 30)
        return 80; // Within a month
    if (daysDiff <= 90)
        return 60; // Within 3 months
    if (daysDiff <= 180)
        return 30; // Within 6 months
    return 10; // More than 6 months apart
};
const calculateBudgetMatch = (auPairRate, hostBudget) => {
    if (!auPairRate || !hostBudget)
        return 50; // Neutral score
    if (auPairRate <= hostBudget)
        return 100;
    ...
    if (overBudgetRatio <= 1.2)
        return 70; // Up to 20% over budget
    if (overBudgetRatio <= 1.5)
        return 40; // Up to 50% over budget
...
const findMatches = async (userId, limit = 20) => {
    ...
    if (user.role === 'AU_PAIR' && user.auPairProfile) {
        ...
        potentialMatches = hostFamilies
            ...
            .map(host => ({
            ...host,
            matchScore: (0, exports.calculateMatchScore)(user.auPairProfile, host.hostFamilyProfile)
        }));
    }
        ...
        potentialMatches = auPairs
            ...
            .map(auPair => ({
            ...auPair,
            matchScore: (0, exports.calculateMatchScore)(auPair.auPairProfile, user.hostFamilyProfile)
        }));
    }
